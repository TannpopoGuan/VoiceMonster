...
@onready var feedback_label = get_node("/root/Level3/GameManager/FeedbackLabel")  # Reference to feedback label
var SpeechRecognizer

# Called when the node enters the scene tree for the first time.
func _ready() -> void:
	SpeechRecognizer = get_node("/root/Level3/GameManager/SpeechRecognizer")
...
func _process(delta):
	if is_defeated:
		return
	#monster_text_label.position = position + Vector2(-1050,-800)
