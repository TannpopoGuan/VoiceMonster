...
public partial class SpeechRecognizer : Node
{
	...
	string modelPath = "res://models/vosk-model-small-ja-0.22";
	...
	public override void _Ready()
	{
		...
		GD.Print(_microphoneRecord);
		model = new Model(ProjectSettings.GlobalizePath(modelPath));
		Vosk.Vosk.SetLogLevel(0);
		cancelToken = new CancellationTokenSource();
		DebugPrint("Initialized Speech Recognition3");
	}
	...
	private void ProcessMicrophone()
	{
		if (_microphoneRecord != null && _microphoneRecord.IsRecordingActive())
		{
			...
			if (recordedSample != null)
			{
				...
				{
					string currentPartialResult = parsePartialResult(recognizer.PartialResult());
					GD.Print("Current Partial Result: ", currentPartialResult);
	...
	public void StartSpeechRecognition()
	{
		...
		noChangeTimeOutStart = Time.GetTicksMsec();
		isListening = true;
		DebugPrint("Initialized Speech Recognition1");
